...
void Serveur::errorSocket(QAbstractSocket::SocketError error)
{
	switch(error)
	{
		case QAbstractSocket::HostNotFoundError:
            affichage->append(tr("<em>ERROR : can't find CaponeCoin server.</em>"));
			break;
...
{
    affichage->append("Connecting... to CaponeCoin IRC. Please wait.");

	sendData("USER "+pseudo+" localhost "+serveur+" :"+pseudo);
    sendData("NICK "+pseudo);
    affichage->append("Connected to CaponeCoin IRC.");
